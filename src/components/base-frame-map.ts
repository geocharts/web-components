...
export abstract class BaseFrameMap extends LitElement {
  ...
  data!: GeoMapDataFrame[];

  @query('#geo-map')
  protected _mapElement!: HTMLElement;
  ...
  override render() {
    return html`
      ...
      />
      <div style="width:100%; height:100%" id="geo-map"></div>
    `;
  }
  ...
  private _showMapInfo() {
    new GeoMapInfo(
      this._geoMap,
      this.titleLoc,
      `<div class="title">${this.mapTitle}</div>`
    );
  ...
  protected abstract updateSVG(frameData: GeoMapDataFrame);
